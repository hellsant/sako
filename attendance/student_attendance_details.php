...
<div class="content">
  <div class="bio-graph-heading">
	...
	$id = '';
   $result = mysqli_query($link,"Select a.sid,a.cid,a.attendance,a.roll_no,a.added_date,s.s_name,s.s_id,s.s_email,s.s_contact,s.s_dob,s.s_address,c.c_name,s.s_roll_no,s.s_gender,s.s_religion,s.s_profile_name,s.s_image from tbl_student_attendance a inner join tbl_add_student s on a.sid = s.s_id inner join tbl_add_class c on a.cid = c.c_id where a.sid='".(int)$_SESSION['objLogin']['s_id']."'");
...
?>
    ...
    <?php
    $result_info = mysqli_query($link,"Select a.sid,a.cid,a.attendance,a.roll_no,a.added_date,s.s_name,s.s_id,s.s_email,s.s_contact,s.s_dob,s.s_address,c.c_name,s.s_roll_no,s.s_gender,s.s_religion,s.s_profile_name,s.s_image,p.p_father_name from tbl_student_attendance a inner join tbl_add_student s on a.sid = s.s_id inner join tbl_add_class c on a.cid = c.c_id inner join tbl_add_parent p on p.p_id = s.parent_id where a.sid='".(int)$_SESSION['objLogin']['s_id']."' and a.cid = '".(int)$_SESSION['objLogin']['s_class_id']."'");

	if($row = mysqli_fetch_array($result_info)){?>
  ...
  <h4 class="exam_table_style">Attendance Information</h4>
    ...
    <table class="table common_sakotable_routine table-bordered table-striped dt-responsive">
      ...
      <tbody>
		...
		$result_att = mysqli_query($link,"Select * from tbl_student_attendance where sid='".(int)$_SESSION['objLogin']['s_id']."' and cid='".(int)$_SESSION['objLogin']['s_class_id']."' and a_month='".(int)date('m')."' and a_year='".(int)date('Y')."' and added_date = '" .$xdate. "' order by aid asc");
		if($row_att = mysqli_fetch_array($result_att)){?>
