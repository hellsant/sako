...
?>
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">Notas Primer Parcial </h4>
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">Notas Segundo Parcial</h4>
    <table class="table common_sakotable_att table-bordered table-striped dt-responsive">
      <thead>
        <tr>
          <th>Materia</th>
      ...
      <tbody>
        <?php
		$id = '';
		$result = mysqli_query($link,"Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.student_id='".(int)$_SESSION['objLogin']['s_id']."' and sm.class_id='".(int)$_SESSION['objLogin']['s_class_id']."' and sm.exam_id='".(int)TERM_2."'");
				while($row = mysqli_fetch_array($result)){
				 $g_name = '';
		 $g_point = '0.00';
		 $gp = getStudentGrade($row['marks'],$link);
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">Notas Examen Final</h4>
    <table class="table common_sakotable_att table-bordered table-striped dt-responsive">
      <thead>
        <tr>
          <th>Materia</th>
      ...
      <tbody>
        <?php
		$id = '';
		$result = mysqli_query($link,"Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.student_id='".(int)$_SESSION['objLogin']['s_id']."' and sm.class_id='".(int)$_SESSION['objLogin']['s_class_id']."' and sm.exam_id='".(int)TERM_3."'");
				while($row = mysqli_fetch_array($result)){
				 $g_name = '';
		 $g_point = '0.00';
		 $gp = getStudentGrade($row['marks'],$link);
  ...
  <div style="padding:10px;" align="center"><a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>s_dashboard.php"><i class="fa fa-home"></i>&nbsp;Ventana Principal</a></div>
</div>
