...
if(isset($_POST['txtLibraryID'])){
	if(isset($_POST['hdn']) && $_POST['hdn'] == '0'){
	  $sql = mysqli_query($link,"INSERT INTO `tbl_book_issue`(`lid`,`bname`,`issue_date`,`return_date`,`fine`,`note`) VALUES ('$_POST[txtLibraryID]','$_POST[ddlBookName]','$_POST[txtIssueDate]','$_POST[txtReturnDate]','$_POST[txtFine]','$_POST[txtNote]')");
	  mysqli_query($sql,$link);
	  
	  $result = mysqli_query($link,"SELECT * FROM tbl_library_book_list where b_id = '" . $_POST['ddlBookName'] . "'");
	  if($row = mysqli_fetch_array($result)){
	  	$old_qty = $row['issue_qty'];
		$old_qty = (int)$old_qty - 1;
		mysqli_query($link,"UPDATE `tbl_library_book_list` SET `issue_qty`='".$old_qty."' WHERE b_id='".$_POST['ddlBookName']."'");
	  }
	...
	$sql =mysqli_query($link,"UPDATE `tbl_book_issue` SET `lid`='".$_POST['txtLibraryID']."',`bname`='".$_POST['ddlBookName']."',`issue_date`='".$_POST['txtIssueDate']."',`return_date`='".$_POST['txtReturnDate']."',`fine`='".$_POST['txtFine']."',`note`='".$_POST['txtNote']."' WHERE issue_id='".$_GET['id']."'");
	  mysqli_query($sql,$link);
	  mysqli_close($link);
...
if(isset($_GET['id']) && $_GET['id'] != ''){
	$result = mysqli_query($link,"SELECT * FROM tbl_book_issue where issue_id = '" . $_GET['id'] . "'");
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
			   <select class="form-control" onchange="getAuthorName(this.value);" id="ddlBookName" name="ddlBookName">
                ...
                <?php 
				  	$result_book = mysqli_query($link,"SELECT * FROM tbl_library_book_list order by b_id ASC");
